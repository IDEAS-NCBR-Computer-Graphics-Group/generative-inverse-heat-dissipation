...
def main():
    ...
    bs = model.blur_schedule
    bs_diff = [np.sqrt(bs[i]**2 - bs[i-1]**2) for  i in range(1,len(bs))]
    ...
    #     print('dct')
    #     blurred_by_dct = dctBlur(t_initial_condition, fwd_steps).float().squeeze().numpy()
    #     delta_blurr = blurred_by_dct - np_gray_image
    #     match_sim_numbers.plot_matrix(blurred_by_dct, title=f'DCT schedule Blurr for sigma = {model.blur_schedule[i]:3.4e}')
    #     match_sim_numbers.plot_matrix(delta_blurr, title=f'delta_blurr for delta sigma = {model.blur_schedule[i]:3.4e}')

    #     print('dct_diff')
    #     blurred_by_dct_diff = dctBlur_diff(torch.Tensor(blurred_by_dct_m1.copy()).unsqueeze(0), fwd_steps).float().squeeze().numpy()
    #     delta_blurr_diff = blurred_by_dct_diff - blurred_by_dct
    #     match_sim_numbers.plot_matrix(blurred_by_dct_diff, title=f'DCT schedule Blurr for delta_sigma = {bs_diff[i]:3.4e}')
    #     match_sim_numbers.plot_matrix(delta_blurr_diff, title=f'delta_blurr for delta sigma = {bs_diff[i]:3.4e}')
    #     # match_sim_numbers.plot_matrix(blu, title=f'delta_blurr for delta sigma = {bs_diff[i]:3.4e}')
    ...
    (dt_array, 
     ...
     dFo_realizable) = match_sim_numbers.calculate_t_niu_array(dFo_array, niu_min, niu_max, L)
    ...
    _ = corruptor._corrupt(t_init, config.solver.n_denoising_steps)
    
    # solver.solve(iterations=sum(dt_array))
    img = corruptor.intermediate_samples[-1]
    blurred_by_lbm = np.rot90(img.numpy().copy(), k=0).squeeze(0) # torch to numpy + rotation
    blurred_by_lbm_all = [map01(i.squeeze(0)) for i in corruptor.intermediate_samples]
    ...
    match_sim_numbers.plot_matrices_in_grid(blurred_by_lbm_all, columns=4)

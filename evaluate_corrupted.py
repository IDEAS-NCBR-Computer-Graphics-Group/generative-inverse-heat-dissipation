...
def calculate_fid(config, dataset_name, experiment_name, param_name,
    ...
    def generate_samples(dummy_argument, train_iter=train_iter,
        ...
        initial_corrupted_sample, *_= sampling.get_initial_corrupted_sample(
        ...
        sampling_fn = sampling.get_sampling_fn_inverse_lbm_ns(
            ...
            intermediate_sample_indices=list(range(n_denoising_steps+1)),
...
def calculate_ELBO(config, dataset_name, experiment_name, param_name,
    ...
    L_K_upbound, L_others, mse_losses = mutils.neg_ELBO_corrupted(
        config, trainloader, testloader, corruptor, sigma, delta, image_size,
        train_size, test_size, model, config.device, num_epochs=1
        )
